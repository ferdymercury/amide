...
data_set_t * data_set_`'m4_Internal_Data_Format`'_0D_SCALING_init(amide_data_t init_value) {
  ...
  temp_data_set->format = `'m4_Internal_Data_Format`';
  temp_data_set->data = data_set_get_data_mem(temp_data_set);
  (*DATA_SET_`'m4_Internal_Data_Format`'_0D_SCALING_POINTER(temp_data_set, i)) = init_value;
...
void data_set_`'m4_Internal_Data_Format`'_initialize_data(data_set_t * data_set, 
  ...
  for (i.t = 0; i.t < data_set->dim.t; i.t++)
	...
	for (i.x = 0; i.x < data_set->dim.x; i.x++) 
	  DATA_SET_`'m4_Internal_Data_Format`'_SET_CONTENT(data_set,i)=0.0;
