...
void volume_UINT_0D_SCALING_recalc_max_min(volume_t * volume) {
  ...
  temp = VOLUME_UINT_0D_SCALING_CONTENTS(volume, voxelpoint_zero);
  ...
  for(i.t = 0; i.t < volume->data_set->dim.t; i.t++) {
	...
	for (i.x = 0; i.x < volume->data_set->dim.x; i.x++) {
	  temp = VOLUME_UINT_0D_SCALING_CONTENTS(volume, i);
	  if (finite(temp)) {
...
void volume_UINT_0D_SCALING_generate_distribution(volume_t * volume) {
  ...
  for (i.t = 0; i.t < volume->data_set->dim.t; i.t++) {
	...
	for (i.y = 0; i.y < volume->data_set->dim.y; i.y++) 
	  for (i.x = 0; i.x < volume->data_set->dim.x; i.x++) {
	    j.x = scale*(VOLUME_UINT_0D_SCALING_CONTENTS(volume,i)-volume->min);
...
volume_t * volume_UINT_0D_SCALING_get_slice(const volume_t * volume,
  ...
  case TRILINEAR:
	...
	for (i.y = 0; i.y < slice->data_set->dim.y; i.y++) {
	  ...
	  for (i.x = 0; i.x < slice->data_set->dim.x; i.x++) {
		...
		box_value[l] = VOLUME_UINT_0D_SCALING_CONTENTS(volume, box_vp[l]);
	      else
		box_value[l] = EMPTY;
	    }
  ...
  default:  
	...
	for (i.y = 0; i.y < slice->data_set->dim.y; i.y++) {
	  ...
	  for (i.x = 0; i.x < slice->data_set->dim.x; i.x++) {
		...
		weight*VOLUME_UINT_0D_SCALING_CONTENTS(volume,j);
