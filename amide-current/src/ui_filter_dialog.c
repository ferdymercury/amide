...
static ui_filter_t * ui_filter_free(ui_filter_t * ui_filter);
static ui_filter_t * ui_filter_init(void);
...
static gboolean next_page_cb(GtkWidget * page, gpointer *druid, gpointer data) {
  ...
  case PICK_FILTER_PAGE:
    gnome_druid_set_page(GNOME_DRUID(druid), 
			 GNOME_DRUID_PAGE(ui_filter->page[GAUSSIAN_FILTER_PAGE+ui_filter->filter]));

    break;
  default:
    gnome_druid_set_page(GNOME_DRUID(druid), 
			 GNOME_DRUID_PAGE(ui_filter->page[OUTPUT_PAGE]));
    break;
  }
...
static gboolean back_page_cb(GtkWidget * page, gpointer *druid, gpointer data) {
  ...
  default:
    gnome_druid_set_page(GNOME_DRUID(druid), 
			 GNOME_DRUID_PAGE(ui_filter->page[PICK_FILTER_PAGE]));
    break;
  }
...
static void prepare_page_cb(GtkWidget * page, gpointer * druid, gpointer data) {
  ...
  if (ui_filter->table[which_page] == NULL) {
	...
	gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);
      ...
      g_signal_connect(G_OBJECT(option_menu), "changed", G_CALLBACK(filter_cb), ui_filter);
...
static void filter_cb(GtkWidget * widget, gpointer data) {
  ...
  ui_filter_t * ui_filter = data;
  ui_filter->filter = gtk_option_menu_get_history(GTK_OPTION_MENU(widget));
  return;
}
...
static void kernel_size_spinner_cb(GtkSpinButton * spin_button, gpointer data) {
  ...
  ui_filter->kernel_size = int_value;
  g_signal_handlers_block_by_func(G_OBJECT(spin_button),
				  G_CALLBACK(kernel_size_spinner_cb), ui_filter);
  gtk_spin_button_set_value(spin_button, ui_filter->kernel_size); 
...
static void fwhm_spinner_cb(GtkSpinButton * spin_button, gpointer data) {
  ...
  ui_filter->fwhm = value;
  g_signal_handlers_block_by_func(G_OBJECT(spin_button),
				  G_CALLBACK(fwhm_spinner_cb), ui_filter);
  gtk_spin_button_set_value(spin_button, ui_filter->fwhm);
  g_signal_handlers_unblock_by_func(G_OBJECT(spin_button),
				    G_CALLBACK(fwhm_spinner_cb), ui_filter);
...
static void finish_cb(GtkWidget* widget, gpointer druid, gpointer data) {
  ...
  filtered = amitk_data_set_get_filtered(ui_filter->data_set, 
  					 ui_filter->filter,
  					 ui_filter->kernel_size,
  					 ui_filter->fwhm);

  /* and add the new data set to the study */
  ui_study_add_data_set(ui_study, filtered); /* this adds a reference to the data set*/
...
static void cancel_cb(GtkWidget* widget, gpointer data) {
  ...
  ui_filter_t * ui_filter = data;
  GtkWidget * dialog = ui_filter->dialog;
  gboolean return_val;
...
static gboolean delete_event(GtkWidget * widget, GdkEvent * event, gpointer data) {
  ...
  /* trash collection */
  ui_filter = ui_filter_free(ui_filter);
...
static ui_filter_t * ui_filter_free(ui_filter_t * ui_filter) {
  ...
  /* sanity checks */
  g_return_val_if_fail(ui_filter != NULL, NULL);
  g_return_val_if_fail(ui_filter->reference_count > 0, NULL);

  /* remove a reference count */
  ui_filter->reference_count--;

  /* things to do if we've removed all reference's */
  if (ui_filter->reference_count == 0) {
#ifdef AMIDE_DEBUG
    g_print("freeing ui_filter\n");
#endif

    if (ui_filter->data_set != NULL) {
      g_object_unref(ui_filter->data_set);
      ui_filter->data_set = NULL;
    }
...
void ui_filter_dialog_create(ui_study_t * ui_study) {
  ...
  gtk_container_add(GTK_CONTAINER(ui_filter->dialog), druid);
  g_object_set_data(G_OBJECT(druid), "ui_study", ui_study);
  g_signal_connect(G_OBJECT(druid), "cancel", G_CALLBACK(cancel_cb), ui_filter);


  for (i_page=PICK_FILTER_PAGE; i_page<OUTPUT_PAGE; i_page++) {
    ui_filter->page[i_page] = gnome_druid_page_standard_new_with_vals(wizard_name,logo,NULL);
    g_object_set_data(G_OBJECT(ui_filter->page[i_page]),"which_page", GINT_TO_POINTER(i_page));
    ...
    g_signal_connect_after(G_OBJECT(ui_filter->page[i_page]), "prepare", 
			   G_CALLBACK(prepare_page_cb), ui_filter);
    g_signal_connect(G_OBJECT(ui_filter->page[i_page]), "next", G_CALLBACK(next_page_cb), ui_filter);
    g_signal_connect(G_OBJECT(ui_filter->page[i_page]), "back", G_CALLBACK(back_page_cb), ui_filter);
    gnome_druid_append_page(GNOME_DRUID(druid), GNOME_DRUID_PAGE(ui_filter->page[i_page]));
  }

    
  /* ----------------  conclusion page ---------------------------------- */
  ui_filter->page[OUTPUT_PAGE] = 
    gnome_druid_page_edge_new_with_vals(GNOME_EDGE_FINISH, TRUE,
					wizard_name,finish_page_text, logo,NULL, NULL);
  g_object_set_data(G_OBJECT(ui_filter->page[i_page]),"which_page", GINT_TO_POINTER(i_page));
  g_signal_connect(G_OBJECT(ui_filter->page[OUTPUT_PAGE]), "finish", G_CALLBACK(finish_cb), ui_filter);
  g_signal_connect(G_OBJECT(ui_filter->page[OUTPUT_PAGE]), "back", G_CALLBACK(back_page_cb), ui_filter);
  gnome_druid_append_page(GNOME_DRUID(druid),  GNOME_DRUID_PAGE(ui_filter->page[OUTPUT_PAGE]));

  g_object_unref(logo);
  gtk_widget_show_all(ui_filter->dialog);
