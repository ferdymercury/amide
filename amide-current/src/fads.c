...
void fads_svd_factors(AmitkDataSet * data_set, 
  ...
  */
  ...
  if (pcomponents != NULL) {
    components = g_try_new(gdouble, n*n);
    if (components == NULL) {
      g_warning("Failed to allocate %d component array", n*n);
      goto ending;
    }
    *pcomponents = components;
    for (i=0; i<n; i++)
      for (j=0; j<n; j++)
	components[i*n+j] = gsl_matrix_get(matrix_v, i, j);
  }
...
gdouble * calc_weights(AmitkDataSet * ds) {
  ...
  /* setup the weights, each frame has a weight proportional to the sqrt of duration */
  total_weight = 0;
  for (t=0; t<num_frames; t++) {
    weight[t] = sqrt(amitk_data_set_get_frame_duration(ds, t));
...
typedef struct pls_params_t {
  ...
  gdouble * norm_factor; /* normalization factors for the penalization constraint */
  gdouble * dot_prod; /* dot products between the different coefficient columns */
...
static void pls_calc_dot_prods(pls_params_t * p, const gsl_vector *v) {
  ...
  for (f=0; f < p->num_factors-1; f++) {
	...
	inner += fabs(gsl_vector_get(v, i+f))*fabs(gsl_vector_get(v, i+q));
      p->dot_prod[f*p->num_factors+q] = inner;
      p->dot_prod[q*p->num_factors+f] = inner;
    }
  }
...
static gdouble pls_calc_function(pls_params_t * p, const gsl_vector *v) {
  ...
  for (f=0; f < p->num_factors-1; f++) {
    for (q=f+1; q < p->num_factors; q++) {
      temp = p->norm_factor[f]*p->norm_factor[q];
      if (temp > EPSILON){  /* guard for division by close to zero */
	orth_answer+=p->dot_prod[f*p->num_factors+q]/temp;
      }
    }
  }
...
void fads_pls(AmitkDataSet * data_set, 
  ...
  //  gsl_multimin_fminimizer * multmin_minimizer = NULL;
  gsl_multimin_function_fdf multmin_func;
  //  gsl_multimin_function multmin_func;
  AmitkVoxel dim;
  pls_params_t p;
  gint iter=0;
  gsl_vector * v=NULL;
  ...
  gdouble new_b;
  gdouble * components;
  gint svd_num_factors;
  gdouble * facts; // remove
  //  gsl_vector *ss; // remove
  ...
  p.norm_factor = g_try_new(gdouble, p.num_factors);
  g_return_if_fail(p.norm_factor != NULL); 

  p.dot_prod = g_try_new(gdouble, p.num_factors*p.num_factors);
  g_return_if_fail(p.dot_prod != NULL);
  /* initialize the diagonal... although it should never get used */
  for (f=0; f<p.num_factors; f++)
    p.dot_prod[f*(1+p.num_factors)] = 0.0;
  ...
  multmin_minimizer = gsl_multimin_fdfminimizer_alloc (gsl_multimin_fdfminimizer_conjugate_pr,
  						       p.num_variables);
  //  multmin_minimizer = gsl_multimin_fminimizer_alloc (gsl_multimin_fminimizer_nmsimplex,
  //						     p.num_variables);
  ...
  fads_svd_factors(p.data_set, &svd_num_factors, NULL, &components);
  facts = g_try_new(gdouble, p.num_factors*p.num_frames); // remove
  for (f=0; f<p.num_factors; f++) {
    for (j=0; j<p.num_frames; j++) {
      temp = components[j*svd_num_factors+f];
      temp *= AMITK_DATA_SET_FRAME_MAX(p.data_set, j);
      gsl_vector_set(v, f*p.num_frames+j, temp);
      facts[f*p.num_frames+j] = temp;
    }
  }
  g_free(components);
  ...
  if (EQUAL_ZERO(p.orth)) {
    ...
    p.b = (b_fraction*p.b*(p.ls+p.neg))/p.orth;

  for (i=0; i<10; i++)
    b_values[i] = p.b;

  //  ss = gsl_vector_alloc(p.num_variables);
  //  gsl_vector_set_all(ss,1.0);

  gsl_multimin_fdfminimizer_set(multmin_minimizer, &multmin_func, v, 0.1, stopping_criteria);
  //  gsl_multimin_fminimizer_set(multmin_minimizer, &multmin_func, v, ss);
  ...
  //  frprmn(v, p.num_variables, stopping_criteria, &p);
  //  status = GSL_SUCCESS;

  do {
      iter++;
      status = gsl_multimin_fdfminimizer_iterate (multmin_minimizer);
      //      status = gsl_multimin_fminimizer_iterate (multmin_minimizer);
	...
	status = gsl_multimin_test_gradient (multmin_minimizer->gradient, stopping_criteria);
      ...
      // remove
      //      if (gsl_multimin_test_size(gsl_multimin_fminimizer_size(multmin_minimizer), 1e-2) == GSL_SUCCESS) {
      //	g_print("converged\n");
      //	break;
      //      }
...
#if AMIDE_DEBUG  
      g_print("iter %d\tsize %f\t%g=%5.3g+%5.3g+%5.3g+%5.3g\tb=%g\n", iter, 
	      100.0*gsl_multimin_fdfminimizer_minimum(multmin_minimizer)/magnitude,
	      //	      100.0*gsl_multimin_fminimizer_minimum(multmin_minimizer)/magnitude, //remove
  ...
  for (j=0; j<p.num_frames; j++) {
    ...
    for (f=0; f<p.num_factors; f++) {
      ...
      //      fprintf(file_pointer, "\t%g", gsl_vector_get(v, f*p.num_frames+j));
 ...
 ending:
  ...
  if (multmin_minimizer != NULL) {
    ...
    //    gsl_multimin_fminimizer_free (multmin_minimizer) ;
  ...
  if (p.norm_factor != NULL) {
    g_free(p.norm_factor);
    p.norm_factor = NULL;
  }
