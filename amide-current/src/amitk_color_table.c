...
static const amide_data_t kViridis[256*3] = { // from https://github.com/RhoInc/sas-viridis/blob/master/src/viridis.sas
...
rgba_t amitk_color_table_lookup(amide_data_t datum, AmitkColorTable which,
  ...
  case AMITK_COLOR_TABLE_INV_NIH_WHITE:
    ...
    rgba.a = 0xFF-rgba.a; 
    break;
  case AMITK_COLOR_TABLE_VIRIDIS:
    temp = ((datum-min)/(max-min)); /* between 0.0 and 1.0 */
    if (temp >= 1.0)
      index = 255;
    else if (temp < 0.0)
    ...
    rgba.r = 0xFF * kViridis[index*3];
    rgba.g = 0xFF * kViridis[index*3+1];
    rgba.b = 0xFF * kViridis[index*3+2];
    rgba.a = index;
    break;
  case AMITK_COLOR_TABLE_INV_VIRIDIS:
      temp = ((datum-min)/(max-min)); /* between 0.0 and 1.0 */
      unsigned short index = 0;
      if (temp >= 1.0)
        index = 255;
