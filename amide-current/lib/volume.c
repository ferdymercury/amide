...
volume_t * volume_init(void) {
  ...
  if ((temp_volume = (volume_t *) g_malloc(sizeof(volume_t))) == NULL) {
    g_warning("Couldn't allocate memory for the volume structure");
    return temp_volume;
  }
  ...
  if ((temp_volume->internal_scaling = data_set_FLOAT_0D_SCALING_init(1.0)) == NULL) {
    g_warning("Couldn't allocate memory for internal scaling structure");
...
volume_t * volume_load_xml(gchar * volume_xml_filename, const gchar * study_directory) {
  ...
  if ((doc = xmlParseFile(volume_xml_filename)) == NULL) {
    g_warning("Couldn't Parse AMIDE volume xml file %s/%s",study_directory,volume_xml_filename);
    return volume_unref(new_volume);
  }

  /* get the root of our document */
  if ((nodes = xmlDocGetRootElement(doc)) == NULL) {
    g_warning("AMIDE volume xml file doesn't appear to have a root: %s/%s",
	      study_directory,volume_xml_filename);
    return volume_unref(new_volume);
  }
  ...
  if (scan_date == NULL) {
    ...
    g_warning("no scan date found in image data, educated guess is %s", scan_date);
  }
...
volume_t * volume_import_file(import_method_t import_method, int submethod,
  ...
  if (import_method == AMIDE_GUESS) {
    ...
    { /* unrecognized file type */
      g_warning("Guessing raw data, as extension %s not recognized on file:\n\t%s", 
  ...
  case PEM_DATA:
    if ((import_volume= pem_data_import(import_filename, model_filename)) == NULL)
      g_warning("Could not interpret as a PEM technologies model file:\n\t%s",model_filename);
    break;
...
volume_t * volume_copy(volume_t * src_volume) {
  ...
  if ((dest_volume->frame_duration == NULL) || 
    ...
    g_warning("couldn't allocate space for the frame duration/max/or min info");
...
void volume_set_scaling(volume_t * volume, amide_data_t new_external_scaling) {
  ...
  if (volume->current_scaling == NULL) {
    if ((volume->current_scaling = data_set_init()) == NULL) {
      g_warning("Couldn't allocate space for current scaling stucture");
      return;
    }
...
void volume_calc_frame_max_min(volume_t * volume) {
  ...
  if (volume->frame_max == NULL) {
    if ((volume->frame_max = volume_get_frame_max_min_mem(volume)) == NULL) {
      g_warning("couldn't allocate space for the frame max info");
      return;
    }
    if ((volume->frame_min = volume_get_frame_max_min_mem(volume)) == NULL) {
      g_warning("couldn't allocate space for the frame min info");
      return;
    }
  }
...
volumes_t * volumes_init(volume_t * volume) {
  ...
  if ((temp_volumes = (volumes_t *) g_malloc(sizeof(volumes_t))) == NULL) {
    g_warning("Couldn't allocate memory for the volume list");
    return temp_volumes;
  }
...
//    g_warning("couldn't allocate space for the axis volume structure");
//    return axis_volume;
//  }
//  if ((axis_volume->data_set = data_set_init()) == NULL) {
//    g_warning("couldn't allocate space for the axis data set structure");
...
//    g_warning("couldn't allocate space for the axis volume frame duration array");
//    return volume_unref(axis_volume);
//  }
