/* gtkdirsel.c - this is just gtkfilesel.c, modified for picking .xif directories */
...
static void gtk_dir_selection_class_init    (GtkDirSelectionClass *klass);
static void gtk_dir_selection_init          (GtkDirSelection      *dirsel);
static void gtk_dir_selection_destroy       (GtkObject             *object);
static gint gtk_dir_selection_key_press     (GtkWidget             *widget,
					      GdkEventKey           *event,
					      gpointer               user_data);

static void gtk_dir_selection_file_button (GtkWidget *widget,
					    gint row, 
					    gint column, 
					    GdkEventButton *bevent,
					    gpointer user_data);

static void gtk_dir_selection_dir_button (GtkWidget *widget,
					   gint row, 
					   gint column, 
					   GdkEventButton *bevent,
					   gpointer data);

static void gtk_dir_selection_populate      (GtkDirSelection      *ds,
					      gchar                 *rel_path,
					      gint                   try_complete);
static void gtk_dir_selection_abort         (GtkDirSelection      *ds);

static void gtk_dir_selection_update_history_menu (GtkDirSelection       *ds,
						    gchar                  *current_dir);

static void gtk_dir_selection_create_dir (GtkWidget *widget, gpointer data);
static void gtk_dir_selection_delete_file (GtkWidget *widget, gpointer data);
static void gtk_dir_selection_rename_file (GtkWidget *widget, gpointer data);
...
GtkType
gtk_dir_selection_get_type (void)
{
  static GtkType dir_selection_type = 0;

  if (!dir_selection_type)
    {
      static const GtkTypeInfo dirsel_info =
      {
	"GtkDirSelection",
	sizeof (GtkDirSelection),
	sizeof (GtkDirSelectionClass),
	(GtkClassInitFunc) gtk_dir_selection_class_init,
	(GtkObjectInitFunc) gtk_dir_selection_init,
	/* reserved_1 */ NULL,
	/* reserved_2 */ NULL,
        (GtkClassInitFunc) NULL,
      };

      dir_selection_type = gtk_type_unique (GTK_TYPE_WINDOW, &dirsel_info);
    }

  return dir_selection_type;
}

static void
gtk_dir_selection_class_init (GtkDirSelectionClass *class)
{
  ...
  object_class->destroy = gtk_dir_selection_destroy;
}

static void
gtk_dir_selection_init (GtkDirSelection *dirsel)
{
  ...
  dirsel->cmpl_state = cmpl_init_state ();

  /* The dialog-sized vertical box  */
  dirsel->main_vbox = gtk_vbox_new (FALSE, 10);
  gtk_container_set_border_width (GTK_CONTAINER (dirsel), 10);
  gtk_container_add (GTK_CONTAINER (dirsel), dirsel->main_vbox);
  gtk_widget_show (dirsel->main_vbox);

  /* The horizontal box containing create, rename etc. buttons */
  dirsel->button_area = gtk_hbutton_box_new ();
  gtk_button_box_set_layout(GTK_BUTTON_BOX(dirsel->button_area), GTK_BUTTONBOX_START);
  gtk_button_box_set_spacing(GTK_BUTTON_BOX(dirsel->button_area), 0);
  gtk_box_pack_start (GTK_BOX (dirsel->main_vbox), dirsel->button_area, 
		      FALSE, FALSE, 0);
  gtk_widget_show (dirsel->button_area);
  
  gtk_dir_selection_show_fileop_buttons(dirsel);

  /* hbox for pulldown menu */
  pulldown_hbox = gtk_hbox_new (TRUE, 5);
  gtk_box_pack_start (GTK_BOX (dirsel->main_vbox), pulldown_hbox, FALSE, FALSE, 0);
  gtk_widget_show (pulldown_hbox);
  
  /* Pulldown menu */
  dirsel->history_pulldown = gtk_option_menu_new ();
  gtk_widget_show (dirsel->history_pulldown);
  gtk_box_pack_start (GTK_BOX (pulldown_hbox), dirsel->history_pulldown, 
		      FALSE, FALSE, 0);
    
  /*  The horizontal box containing the directory and file listboxes  */
  list_hbox = gtk_hbox_new (FALSE, 5);
  gtk_box_pack_start (GTK_BOX (dirsel->main_vbox), list_hbox, TRUE, TRUE, 0);
  gtk_widget_show (list_hbox);

  /* The directories clist */
  dir_title[0] = "Directories";
  dir_title[1] = NULL;
  dirsel->dir_list = gtk_clist_new_with_titles (1, (gchar**) dir_title);
  gtk_widget_set_usize (dirsel->dir_list, DIR_LIST_WIDTH, DIR_LIST_HEIGHT);
  gtk_signal_connect (GTK_OBJECT (dirsel->dir_list), "select_row",
		      (GtkSignalFunc) gtk_dir_selection_dir_button, 
		      (gpointer) dirsel);
  gtk_clist_column_titles_passive (GTK_CLIST (dirsel->dir_list));

  scrolled_win = gtk_scrolled_window_new (NULL, NULL);
  gtk_container_add (GTK_CONTAINER (scrolled_win), dirsel->dir_list);
  gtk_scrolled_window_set_policy (GTK_SCROLLED_WINDOW (scrolled_win),
				  GTK_POLICY_AUTOMATIC, GTK_POLICY_ALWAYS);
  gtk_container_set_border_width (GTK_CONTAINER (scrolled_win), 5);
  gtk_box_pack_start (GTK_BOX (list_hbox), scrolled_win, TRUE, TRUE, 0);
  gtk_widget_show (dirsel->dir_list);
  gtk_widget_show (scrolled_win);

  /* The files clist */
  file_title[0] = "Files";
  file_title[1] = NULL;
  dirsel->file_list = gtk_clist_new_with_titles (1, (gchar**) file_title);
  gtk_widget_set_usize (dirsel->file_list, FILE_LIST_WIDTH, FILE_LIST_HEIGHT);
  gtk_signal_connect (GTK_OBJECT (dirsel->file_list), "select_row",
		      (GtkSignalFunc) gtk_dir_selection_file_button, 
		      (gpointer) dirsel);
  gtk_clist_column_titles_passive (GTK_CLIST (dirsel->file_list));

  scrolled_win = gtk_scrolled_window_new (NULL, NULL);
  gtk_container_add (GTK_CONTAINER (scrolled_win), dirsel->file_list);
  gtk_scrolled_window_set_policy (GTK_SCROLLED_WINDOW (scrolled_win),
				  GTK_POLICY_AUTOMATIC, GTK_POLICY_ALWAYS);
  gtk_container_set_border_width (GTK_CONTAINER (scrolled_win), 5);
  gtk_box_pack_start (GTK_BOX (list_hbox), scrolled_win, TRUE, TRUE, 0);
  gtk_widget_show (dirsel->file_list);
  gtk_widget_show (scrolled_win);

  /* action area for packing buttons into. */
  dirsel->action_area = gtk_hbox_new (TRUE, 0);
  gtk_box_pack_start (GTK_BOX (dirsel->main_vbox), dirsel->action_area, 
		      FALSE, FALSE, 0);
  gtk_widget_show (dirsel->action_area);
  
  /*  The OK/Cancel button area */
  confirm_area = gtk_hbutton_box_new ();
  gtk_button_box_set_layout(GTK_BUTTON_BOX(confirm_area), GTK_BUTTONBOX_END);
  gtk_button_box_set_spacing(GTK_BUTTON_BOX(confirm_area), 5);
  gtk_box_pack_end (GTK_BOX (dirsel->main_vbox), confirm_area, FALSE, FALSE, 0);
  gtk_widget_show (confirm_area);

  /*  The OK button  */
  dirsel->ok_button = gtk_button_new_with_label ("OK");
  GTK_WIDGET_SET_FLAGS (dirsel->ok_button, GTK_CAN_DEFAULT);
  gtk_box_pack_start (GTK_BOX (confirm_area), dirsel->ok_button, TRUE, TRUE, 0);
  gtk_widget_grab_default (dirsel->ok_button);
  gtk_widget_show (dirsel->ok_button);

  /*  The Cancel button  */
  dirsel->cancel_button = gtk_button_new_with_label ("Cancel");
  GTK_WIDGET_SET_FLAGS (dirsel->cancel_button, GTK_CAN_DEFAULT);
  gtk_box_pack_start (GTK_BOX (confirm_area), dirsel->cancel_button, TRUE, TRUE, 0);
  gtk_widget_show (dirsel->cancel_button);

  /*  The selection entry widget  */
  entry_vbox = gtk_vbox_new (FALSE, 2);
  gtk_box_pack_end (GTK_BOX (dirsel->main_vbox), entry_vbox, FALSE, FALSE, 0);
  gtk_widget_show (entry_vbox);

  dirsel->selection_text = label = gtk_label_new ("");
  gtk_misc_set_alignment (GTK_MISC (label), 0.0, 0.5);
  gtk_box_pack_start (GTK_BOX (entry_vbox), label, FALSE, FALSE, 0);
  gtk_widget_show (label);

  dirsel->selection_entry = gtk_entry_new ();
  gtk_signal_connect (GTK_OBJECT (dirsel->selection_entry), "key_press_event",
		      (GtkSignalFunc) gtk_dir_selection_key_press, dirsel);
  gtk_signal_connect_object (GTK_OBJECT (dirsel->selection_entry), "focus_in_event",
			     (GtkSignalFunc) gtk_widget_grab_default,
			     GTK_OBJECT (dirsel->ok_button));
  gtk_signal_connect_object (GTK_OBJECT (dirsel->selection_entry), "activate",
                             (GtkSignalFunc) gtk_button_clicked,
                             GTK_OBJECT (dirsel->ok_button));
  gtk_box_pack_start (GTK_BOX (entry_vbox), dirsel->selection_entry, TRUE, TRUE, 0);
  gtk_widget_show (dirsel->selection_entry);

  if (!cmpl_state_okay (dirsel->cmpl_state))
    {
      gchar err_buf[256];

      sprintf (err_buf, "Directory unreadable: %s", cmpl_strerror (cmpl_errno));

      gtk_label_set_text (GTK_LABEL (dirsel->selection_text), err_buf);
    }
  else
    {
      gtk_dir_selection_populate (dirsel, "", FALSE);
    }

  gtk_widget_grab_focus (dirsel->selection_entry);
}

GtkWidget*
gtk_dir_selection_new (const gchar *title)
{
  GtkDirSelection *dirsel;

  dirsel = gtk_type_new (GTK_TYPE_DIR_SELECTION);
  gtk_window_set_title (GTK_WINDOW (dirsel), title);

  return GTK_WIDGET (dirsel);
}

void
gtk_dir_selection_show_fileop_buttons (GtkDirSelection *dirsel)
{
  g_return_if_fail (dirsel != NULL);
  g_return_if_fail (GTK_IS_DIR_SELECTION (dirsel));
    
  /* delete, create directory, and rename */
  if (!dirsel->fileop_c_dir) 
    {
      dirsel->fileop_c_dir = gtk_button_new_with_label ("Create Dir");
      gtk_signal_connect (GTK_OBJECT (dirsel->fileop_c_dir), "clicked",
			  (GtkSignalFunc) gtk_dir_selection_create_dir, 
			  (gpointer) dirsel);
      gtk_box_pack_start (GTK_BOX (dirsel->button_area), 
			  dirsel->fileop_c_dir, TRUE, TRUE, 0);
      gtk_widget_show (dirsel->fileop_c_dir);
    }
	
  if (!dirsel->fileop_del_file) 
    {
      dirsel->fileop_del_file = gtk_button_new_with_label ("Delete File");
      gtk_signal_connect (GTK_OBJECT (dirsel->fileop_del_file), "clicked",
			  (GtkSignalFunc) gtk_dir_selection_delete_file, 
			  (gpointer) dirsel);
      gtk_box_pack_start (GTK_BOX (dirsel->button_area), 
			  dirsel->fileop_del_file, TRUE, TRUE, 0);
      gtk_widget_show (dirsel->fileop_del_file);
    }

  if (!dirsel->fileop_ren_file)
    {
      dirsel->fileop_ren_file = gtk_button_new_with_label ("Rename File");
      gtk_signal_connect (GTK_OBJECT (dirsel->fileop_ren_file), "clicked",
			  (GtkSignalFunc) gtk_dir_selection_rename_file, 
			  (gpointer) dirsel);
      gtk_box_pack_start (GTK_BOX (dirsel->button_area), 
			  dirsel->fileop_ren_file, TRUE, TRUE, 0);
      gtk_widget_show (dirsel->fileop_ren_file);
    }

  gtk_widget_queue_resize(GTK_WIDGET(dirsel));
}

void       
gtk_dir_selection_hide_fileop_buttons (GtkDirSelection *dirsel)
{
  g_return_if_fail (dirsel != NULL);
  g_return_if_fail (GTK_IS_DIR_SELECTION (dirsel));
  ...
  if (dirsel->fileop_ren_file) 
    {
      gtk_widget_destroy (dirsel->fileop_ren_file);
      dirsel->fileop_ren_file = NULL;
    }

  if (dirsel->fileop_del_file)
    {
      gtk_widget_destroy (dirsel->fileop_del_file);
      dirsel->fileop_del_file = NULL;
    }

  if (dirsel->fileop_c_dir)
    {
      gtk_widget_destroy (dirsel->fileop_c_dir);
      dirsel->fileop_c_dir = NULL;
    }
}
...
void
gtk_dir_selection_set_filename (GtkDirSelection *dirsel,
				 const gchar      *filename)
{
  ...
  gtk_dir_selection_populate (dirsel, buf, FALSE);

  if (dirsel->selection_entry)
    gtk_entry_set_text (GTK_ENTRY (dirsel->selection_entry), name);
}

gchar*
gtk_dir_selection_get_filename (GtkDirSelection *dirsel)
{
  static char nothing[2] = "";
  char *text;
  char *filename;

  g_return_val_if_fail (dirsel != NULL, nothing);
  g_return_val_if_fail (GTK_IS_DIR_SELECTION (dirsel), nothing);

  text = gtk_entry_get_text (GTK_ENTRY (dirsel->selection_entry));
  if (text)
    {
      filename = cmpl_completion_fullname (text, dirsel->cmpl_state);
      return filename;
    }
...
void
gtk_dir_selection_complete (GtkDirSelection *dirsel,
			     const gchar      *pattern)
{
  g_return_if_fail (dirsel != NULL);
  g_return_if_fail (GTK_IS_DIR_SELECTION (dirsel));
  ...
  if (dirsel->selection_entry)
    gtk_entry_set_text (GTK_ENTRY (dirsel->selection_entry), pattern);
  gtk_dir_selection_populate (dirsel, (gchar*) pattern, TRUE);
}

static void
gtk_dir_selection_destroy (GtkObject *object)
{
  GtkDirSelection *dirsel;
  GList *list;
  ...
  g_return_if_fail (GTK_IS_DIR_SELECTION (object));

  dirsel = GTK_DIR_SELECTION (object);
  
  if (dirsel->fileop_dialog)
	  gtk_widget_destroy (dirsel->fileop_dialog);
  
  if (dirsel->history_list)
    {
      list = dirsel->history_list;
      while (list)
	{
      ...
      g_list_free (dirsel->history_list);
      dirsel->history_list = NULL;
    }
  
  cmpl_free_state (dirsel->cmpl_state);
  dirsel->cmpl_state = NULL;
...
static void
gtk_dir_selection_fileop_error (GtkDirSelection *ds, gchar *error_message)
{
  GtkWidget *label;
  GtkWidget *vbox;
  GtkWidget *button;
  ...
  gtk_signal_connect (GTK_OBJECT (dialog), "destroy",
		      (GtkSignalFunc) gtk_dir_selection_fileop_destroy, 
		      (gpointer) ds);
  */
...
static void
gtk_dir_selection_fileop_destroy (GtkWidget *widget, gpointer data)
{
  GtkDirSelection *ds = data;

  g_return_if_fail (ds != NULL);
  g_return_if_fail (GTK_IS_DIR_SELECTION (ds));
  
  ds->fileop_dialog = NULL;
}


static void
gtk_dir_selection_create_dir_confirmed (GtkWidget *widget, gpointer data)
{
  GtkDirSelection *ds = data;
  gchar *dirname;
  gchar *path;
  ...
  dirname = gtk_entry_get_text (GTK_ENTRY (ds->fileop_entry));
  cmpl_state = (CompletionState*) ds->cmpl_state;
  ...
  if ( (mkdir (full_path, 0755) < 0) ) 
    {
      ...
      gtk_dir_selection_fileop_error (ds, buf);
    }
  g_free (full_path);
  
  gtk_widget_destroy (ds->fileop_dialog);
  gtk_dir_selection_populate (ds, "", FALSE);
}
  
static void
gtk_dir_selection_create_dir (GtkWidget *widget, gpointer data)
{
  GtkDirSelection *ds = data;
  GtkWidget *label;
  GtkWidget *dialog;
  ...
  /* main dialog */
  ds->fileop_dialog = dialog = gtk_dialog_new ();
  gtk_signal_connect (GTK_OBJECT (dialog), "destroy",
		      (GtkSignalFunc) gtk_dir_selection_fileop_destroy, 
		      (gpointer) ds);
  ...
  ds->fileop_entry = gtk_entry_new ();
  gtk_box_pack_start (GTK_BOX (vbox), ds->fileop_entry, 
		      TRUE, TRUE, 5);
  GTK_WIDGET_SET_FLAGS(ds->fileop_entry, GTK_CAN_DEFAULT);
  gtk_widget_show (ds->fileop_entry);
  
  /* buttons */
  button = gtk_button_new_with_label ("Create");
  gtk_signal_connect (GTK_OBJECT (button), "clicked",
		      (GtkSignalFunc) gtk_dir_selection_create_dir_confirmed, 
		      (gpointer) ds);
  gtk_box_pack_start(GTK_BOX(GTK_DIALOG(dialog)->action_area),
		     button, TRUE, TRUE, 0);
  GTK_WIDGET_SET_FLAGS(button, GTK_CAN_DEFAULT);
...
static void
gtk_dir_selection_delete_file_confirmed (GtkWidget *widget, gpointer data)
{
  GtkDirSelection *ds = data;
  CompletionState *cmpl_state;
  gchar *path;
  ...
  full_path = g_strconcat (path, "/", ds->fileop_file, NULL);
  ...
  if (stat(full_path, &file_info) == 0) {
	...
	temp_string = 
	  g_strdup_printf("%s%s%s", full_path,G_DIR_SEPARATOR_S, directory_entry->d_name);

	if (fnmatch("*.xml",directory_entry->d_name,FNM_NOESCAPE) == 0) 
	  if (unlink(temp_string) != 0) {
	    buf = g_strconcat("Couldn't unlink file: ",temp_string, NULL);
	    gtk_dir_selection_fileop_error (ds, buf);
	  }
	...
	buf = g_strconcat ("Error deleting file \"", ds->fileop_file, "\":  ", 
			   g_strerror(errno), NULL);
	gtk_dir_selection_fileop_error (ds, buf);
      }
    }
...
static void
gtk_dir_selection_delete_file (GtkWidget *widget, gpointer data)
{
  GtkDirSelection *ds = data;
  GtkWidget *label;
  GtkWidget *vbox;
  ...
  filename = gtk_entry_get_text (GTK_ENTRY (ds->selection_entry));
  if (strlen(filename) < 1)
	  return;

  ds->fileop_file = filename;
  ...
  gtk_signal_connect (GTK_OBJECT (button), "clicked",
		      (GtkSignalFunc) gtk_dir_selection_delete_file_confirmed, 
		      (gpointer) ds);
  gtk_box_pack_start(GTK_BOX(GTK_DIALOG(dialog)->action_area),
		     button, TRUE, TRUE, 0);
  GTK_WIDGET_SET_FLAGS(button, GTK_CAN_DEFAULT);
...
static void
gtk_dir_selection_rename_file_confirmed (GtkWidget *widget, gpointer data)
{
  GtkDirSelection *ds = data;
  gchar *buf;
  gchar *file;
  ...
  file = gtk_entry_get_text (GTK_ENTRY (ds->fileop_entry));
  cmpl_state = (CompletionState*) ds->cmpl_state;
  path = cmpl_reference_position (cmpl_state);
  
  new_filename = g_strconcat (path, "/", file, NULL);
  old_filename = g_strconcat (path, "/", ds->fileop_file, NULL);
...
static void
gtk_dir_selection_rename_file (GtkWidget *widget, gpointer data)
{
  GtkDirSelection *ds = data;
  GtkWidget *label;
  GtkWidget *dialog;
  ...
  ds->fileop_file = gtk_entry_get_text (GTK_ENTRY (ds->selection_entry));
  if (strlen(ds->fileop_file) < 1)
	  return;
  ...
  buf = g_strconcat ("Rename file \"", ds->fileop_file, "\" to:", NULL);
  label = gtk_label_new(buf);
  gtk_misc_set_alignment(GTK_MISC(label), 0.0, 0.0);
  gtk_box_pack_start(GTK_BOX(vbox), label, FALSE, FALSE, 5);
  gtk_widget_show(label);
  ...
  gtk_entry_set_text (GTK_ENTRY (ds->fileop_entry), ds->fileop_file);
  gtk_editable_select_region (GTK_EDITABLE (ds->fileop_entry),
			      0, strlen (ds->fileop_file));

  /* buttons */
  button = gtk_button_new_with_label ("Rename");
  gtk_signal_connect (GTK_OBJECT (button), "clicked",
		      (GtkSignalFunc) gtk_dir_selection_rename_file_confirmed, 
		      (gpointer) ds);
  gtk_box_pack_start(GTK_BOX(GTK_DIALOG(dialog)->action_area),
		     button, TRUE, TRUE, 0);
  GTK_WIDGET_SET_FLAGS(button, GTK_CAN_DEFAULT);
...
static gint
gtk_dir_selection_key_press (GtkWidget   *widget,
			      GdkEventKey *event,
			      gpointer     user_data)
{
  GtkDirSelection *ds;
  ...
  if (event->keyval == GDK_Tab)
    {
      ds = GTK_DIR_SELECTION (user_data);
      text = gtk_entry_get_text (GTK_ENTRY (ds->selection_entry));

      text = g_strdup (text);

      gtk_dir_selection_populate (ds, text, TRUE);
...
static void
gtk_dir_selection_history_callback (GtkWidget *widget, gpointer data)
{
  GtkDirSelection *ds = data;
  HistoryCallbackArg *callback_arg;
  GList *list;
  ...
  while (list) {
    ...
    if (callback_arg->menu_item == widget)
      {
	gtk_dir_selection_populate (ds, callback_arg->directory, FALSE);
	break;
      }
...
static void 
gtk_dir_selection_update_history_menu (GtkDirSelection *ds,
					gchar *current_directory)
{
  ...
  if (ds->history_menu) 
	...
	list = list->next;
      ...
      g_list_free (ds->history_list);
      ds->history_list = NULL;
      
      gtk_widget_destroy (ds->history_menu);
    }
  
  ds->history_menu = gtk_menu_new();
  ...
  for (i = dir_len; i >= 0; i--)
      ...
      if ( (current_dir[i] == '/') || (i == dir_len))
	{
	   ...
	   * (current) path to "" which just refreshes the dirsel */
	  if (dir_len == i) {
	  ...
	  ds->history_list = g_list_append (ds->history_list, callback_arg);
	  
	  gtk_signal_connect (GTK_OBJECT (menu_item), "activate",
			      (GtkSignalFunc) gtk_dir_selection_history_callback,
			      (gpointer) ds);
	  gtk_menu_append (GTK_MENU (ds->history_menu), menu_item);
	  gtk_widget_show (menu_item);
	}
    }

  gtk_option_menu_set_menu (GTK_OPTION_MENU (ds->history_pulldown), 
			    ds->history_menu);
  g_free (current_dir);
}

static void
gtk_dir_selection_file_button (GtkWidget *widget,
			       gint row, 
			       gint column, 
			       GdkEventButton *bevent,
			       gpointer user_data)
{
  GtkDirSelection *ds = NULL;
  ...
  gtk_clist_get_text (GTK_CLIST (ds->file_list), row, 0, &temp);
  ...
  if (filename)
	...
	switch (bevent->type)
	  ...
	  case GDK_2BUTTON_PRESS:
	    gtk_button_clicked (GTK_BUTTON (ds->ok_button));
	    break;
	    
	  default:
	    gtk_entry_set_text (GTK_ENTRY (ds->selection_entry), filename);
	    break;
	  }
      else
	gtk_entry_set_text (GTK_ENTRY (ds->selection_entry), filename);
...
static void
gtk_dir_selection_dir_button (GtkWidget *widget,
			       gint row, 
			       gint column, 
			       GdkEventButton *bevent,
			       gpointer user_data)
{
  ...
  gtk_clist_get_text (GTK_CLIST (ds->dir_list), row, 0, &temp);
  filename = g_strdup (temp);
  ...
  if (filename)
	...
	switch (bevent->type)
	  {
	  case GDK_2BUTTON_PRESS:
	    gtk_dir_selection_populate (ds, filename, FALSE);
	    break;
...
static void
gtk_dir_selection_populate (GtkDirSelection *ds,
			     gchar            *rel_path,
			     gint              try_complete)
{
  CompletionState *cmpl_state;
  ...
  if (!cmpl_state_okay (cmpl_state))
    {
      ...
      gtk_dir_selection_abort (ds);
      return;
    }

  g_assert (cmpl_state->reference_dir);

  gtk_clist_freeze (GTK_CLIST (ds->dir_list));
  gtk_clist_clear (GTK_CLIST (ds->dir_list));
  gtk_clist_freeze (GTK_CLIST (ds->file_list));
  gtk_clist_clear (GTK_CLIST (ds->file_list));

  /* Set the dir_list to include ./ and ../ */
  text[1] = NULL;
  text[0] = "./";
  row = gtk_clist_append (GTK_CLIST (ds->dir_list), text);

  text[0] = "../";
  row = gtk_clist_append (GTK_CLIST (ds->dir_list), text);

  /*reset the max widths of the lists*/
  dir_list_width = gdk_string_width(ds->dir_list->style->font,"../");
  gtk_clist_set_column_width(GTK_CLIST(ds->dir_list),0,dir_list_width);
  file_list_width = 1;
  gtk_clist_set_column_width(GTK_CLIST(ds->file_list),0,file_list_width);
          ...
          if (cmpl_is_directory (poss))
            {
              if (strcmp (filename, "./") != 0 &&
                  strcmp (filename, "../") != 0)
		{
		  int width = gdk_string_width(ds->dir_list->style->font, filename);
		  gchar ** frags1;
		  gchar ** frags2;
		  ...
		  if (!xif_directory) {
			...
			gtk_clist_set_column_width(GTK_CLIST(ds->dir_list),0, width);
		    }
		  } else { /* .xif directory */
		    row = gtk_clist_append (GTK_CLIST (ds->file_list), text);
		    if(width > file_list_width) {
			file_list_width = width;
			gtk_clist_set_column_width(GTK_CLIST(ds->file_list),0, width);
		      }
		  }
	        }
            }
  ...
  gtk_clist_thaw (GTK_CLIST (ds->dir_list));
  gtk_clist_thaw (GTK_CLIST (ds->file_list));
  ...
  if (try_complete)
    {
      ...
      if (cmpl_updated_text (cmpl_state)[0])
        {
	      ...
	      gchar* dir_name = g_strdup (cmpl_updated_text (cmpl_state));
              ...
              gtk_dir_selection_populate (ds, dir_name, TRUE);

              g_free (dir_name);
            }
          else
            {
	      if (ds->selection_entry)
		      gtk_entry_set_text (GTK_ENTRY (ds->selection_entry),
	  ...
	  if (ds->selection_entry)
	    gtk_entry_set_text (GTK_ENTRY (ds->selection_entry), rem_path);
        }
    }
  else
    {
  ...
  if (!did_recurse)
    {
      if (ds->selection_entry)
	gtk_entry_set_position (GTK_ENTRY (ds->selection_entry), selection_index);

      if (ds->selection_entry)
	{
	  sel_text = g_strconcat ("Selection: ",
				  cmpl_reference_position (cmpl_state),
				  NULL);

	  gtk_label_set_text (GTK_LABEL (ds->selection_text), sel_text);
	  g_free (sel_text);
	}

      if (ds->history_pulldown) 
	{
	  gtk_dir_selection_update_history_menu (ds, cmpl_reference_position (cmpl_state));
	}
...
static void
gtk_dir_selection_abort (GtkDirSelection *ds)
{
  gchar err_buf[256];
  ...
  if (ds->selection_entry)
    gtk_label_set_text (GTK_LABEL (ds->selection_text), err_buf);
}
