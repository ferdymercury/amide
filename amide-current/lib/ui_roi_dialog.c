...
void ui_roi_dialog_update_entries(GtkWidget * dialog, realpoint_t new_center) {
...
void ui_roi_dialog_update(GtkWidget * dialog, roi_t * roi) {
  ...
  amitk_coord_frame_edit_change_coords(AMITK_COORD_FRAME_EDIT(cfe), roi->coord_frame);
  amitk_coord_frame_edit_change_center(AMITK_COORD_FRAME_EDIT(cfe), roi_center(roi));

  center = realspace_base_coord_to_alt(roi_center(roi), 
				       study_coord_frame(ui_study->study));
  ui_roi_dialog_update_entries(dialog, center);
