...
void ui_align_pt_dialog_cb_apply(GtkWidget* dialog, gint page_number, gpointer data) {
  ...
  for (i_view_mode=0; i_view_mode <= ui_study->view_mode; i_view_mode++) {
    ...
    amitk_tree_update_object(AMITK_TREE(ui_study->tree[i_view_mode]), old_info, ALIGN_PT);

    /* redraw the alignment point */
    for (i_view=0;i_view<NUM_VIEWS;i_view++)
      amitk_canvas_update_object(AMITK_CANVAS(ui_study->canvas[i_view_mode][i_view]), old_info, ALIGN_PT);
  }
