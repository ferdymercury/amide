...
static void canvas_class_init (AmitkCanvasClass *klass)
{
  ...
  canvas_signals[OBJECT_CHANGED] =
    gtk_signal_new ("object_changed",
  		    GTK_RUN_FIRST,
  		    object_class->type,
  		    GTK_SIGNAL_OFFSET (AmitkCanvasClass, object_changed),
...
static realpoint_t cp_2_rp(AmitkCanvas * canvas, canvaspoint_t canvas_cp) {
  ...
  if (canvas_rp.x < 0.0) canvas_rp.x = 0.0;
  if (canvas_rp.y < 0.0) canvas_rp.y = 0.0;
  if (canvas_rp.z < 0.0) canvas_rp.z = 0.0;
  if (canvas_rp.x > canvas->corner.x) canvas_rp.x = canvas->corner.x;
  if (canvas_rp.y > canvas->corner.y) canvas_rp.y = canvas->corner.y;
  if (canvas_rp.z > canvas->corner.z) canvas_rp.z = canvas->corner.z;
...
static gboolean canvas_event_cb(GtkWidget* widget,  GdkEvent * event, gpointer data) {
  ...
  case CANVAS_EVENT_PRESS_NEW_ROI:
    ...
    default:
      ...
      g_error("unexpected case in %s at line %d, roi_type %d", 
	      __FILE__, __LINE__, undrawn_roi->type);
      break;
    }
    break;
  ...
  case CANVAS_EVENT_RELEASE_ALIGN_PT:
    ...
    if (!REALPOINT_EQUAL(align_pt->point, temp_rp[0])) {
      ...
      gtk_signal_emit(GTK_OBJECT (canvas), canvas_signals[VIEW_CHANGED], 
		      &(canvas->center), canvas->thickness);
      gtk_signal_emit(GTK_OBJECT(canvas), canvas_signals[OBJECT_CHANGED], align_pt, ALIGN_PT);
    }
    break;
  ...
  case CANVAS_EVENT_RELEASE_NEW_ROI:
	...
	rp_abs(realspace_alt_coord_to_alt(temp_rp[1], canvas->coord_frame,
      ...
      gtk_signal_emit(GTK_OBJECT (canvas), canvas_signals[OBJECT_CHANGED], undrawn_roi, ROI);
      break;
    case ISOCONTOUR_2D: 
    case ISOCONTOUR_3D:
  ...
  case CANVAS_EVENT_RELEASE_SHIFT_ROI:
    ...
    if (!REALPOINT_EQUAL(shift, zero_rp)) {
      ...
      canvas->center = roi_center(roi);
      canvas_update(canvas, UPDATE_ALL);
      gtk_signal_emit(GTK_OBJECT (canvas), canvas_signals[OBJECT_CHANGED], roi, ROI);
      gtk_signal_emit(GTK_OBJECT (canvas), canvas_signals[VIEW_CHANGED], 
		      &(canvas->center), canvas->thickness);
    }
    break;
  ...
  case STUDY:
    {
      ...
      g_return_if_fail(parent_object == NULL);

      amitk_canvas_set_coord_frame(canvas, study_coord_frame(study), FALSE);
      amitk_canvas_set_center(canvas, study_view_center(study), FALSE);
      amitk_canvas_set_thickness(canvas, study_view_thickness(study), FALSE);
      amitk_canvas_set_voxel_dim(canvas, volumes_max_min_voxel_size(study_volumes(study)), FALSE);
      amitk_canvas_set_zoom(canvas, study_zoom(study), FALSE);
      amitk_canvas_set_start_time(canvas, study_view_time(study), FALSE);
      amitk_canvas_set_duration(canvas, study_view_duration(study), FALSE);
      amitk_canvas_set_interpolation(canvas, study_interpolation(study), FALSE);
      canvas_update(canvas, UPDATE_ALL);
    }
    break;
  case VOLUME:
  ...
  case STUDY:
    {
      ...
      while (volumes != NULL) {
	amitk_canvas_remove_object(canvas, volumes->volume, VOLUME, FALSE);
	volumes = volumes->next;
      }

      while (rois != NULL) {
	amitk_canvas_remove_object(canvas, rois->roi, ROI, FALSE);
	rois = rois->next;
      }
